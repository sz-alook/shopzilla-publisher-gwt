...
public abstract class ApplicationEntityTypesProcessor<T> {
    ...
    public static Set<java.lang.Class<? extends com.google.gwt.requestfactory.shared.EntityProxy>> getAll() {
        ...
        rtn.add(ProductProxy.class);
        return Collections.unmodifiableSet(rtn);
    }

    private static void process(com.shopzilla.publisher.rich.client.managed.request.ApplicationEntityTypesProcessor<?> processor, Class<?> clazz) {
        if (ProductProxy.class.equals(clazz)) {
            processor.handleProduct((ProductProxy) null);
            return;
        }
        processor.handleNonProxy(null);
    }

    private static void process(com.shopzilla.publisher.rich.client.managed.request.ApplicationEntityTypesProcessor<?> processor, Object proxy) {
        if (proxy instanceof ProductProxy) {
            processor.handleProduct((ProductProxy) proxy);
            return;
        }
    ...
    public abstract void handleProduct(ProductProxy proxy);
